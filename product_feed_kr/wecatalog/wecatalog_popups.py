...
def popups_scrape_skip_reason(
    pop_resp: Any,
    *,
    errcode: int | None = None,
) -> str | None:
    """若应写入 ``pf_scrape_skip`` 则返回 reason；登录过期等可重试错误返回 None。"""
    if errcode == POPUPS_ERR_LOGIN_EXPIRED:
        return None
    if isinstance(pop_resp, dict) and pop_resp.get("error") is not None and errcode is None:
        return "popups_fetch_error"
    if popups_skip_permanent(errcode):
        return "popups_invalid"
    if errcode is not None:
        return "popups_error"
    if isinstance(pop_resp, dict) and not popups_response_ready(pop_resp):
        return "popups_error"
    return None
...
def extract_format_options_from_commodity(com: dict[str, Any] | None) -> dict[str, list[str]]:
    """从 commodity 或列表项 dict 提取尺码/颜色（``formatType`` 1=尺码、2=颜色）。"""
    ...
    for f in formats:
        ...
        if ft == 2 or str(ft) == "2":
            colors.append(name)
        elif ft == 1 or str(ft) == "1" or not multi_dim:
            sizes.append(name)
        else:
            sizes.append(name)
...
def extract_format_options(resp: dict[str, Any] | None) -> dict[str, list[str]]:
    ...
    return extract_format_options_from_commodity(popups_commodity(resp))
